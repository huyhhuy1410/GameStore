...
<div id="page1">
                        <p class="tieude"><?php 
                                                   ...
                                                   $sql     =       "select * from tblsubject where id_subject= " . $_GET['id'];
                                                   $rs      =       mysqli_query($conn, $sql);
                                                   while ($row = mysqli_fetch_array($rs)) 
                                                   $name_subject = $row['name_subject'];

                                                   echo "<span style='text-transform: uppercase;'>".$name_subject."</span>";?>
                                                    </p>
                        <?php
                           include("../include/connect.inc");
                           $sql     =       "select * from tblgame where id_subject=".$_GET['id'];
                           $rs      =       mysqli_query($conn, $sql);
           ...
           /*                $abc          =       mysqli_query($conn,$sql);
                           ...
                           $sql         =       "select * from tblgame where id_subject='".$_GET['id']."' limit $pos, $pageSize"  ;
                           $rs          =       mysqli_query($conn,$sql);
                           $i           =       1;
